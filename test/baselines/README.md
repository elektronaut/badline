...
they stand, so the guard is the diff, not the pass count.
...
family grinds through thousands of keypress halts); the SID suite is the
odd one out at about four minutes. CI runs them only when a push to `main`
touches `lib/`, the runners, the baselines or the Rakefile, plus on demand
from the Actions tab — never on a pull request, and never on a schedule.

Reading a diff:

- `-…PASS` / `+…FAIL` is a regression; the reverse is a fix.
- A changed `diff=NNNpx` means a screenshot moved without changing verdict.
- A `FAIL<TAB>timeout` in `sid.txt` is a test that never wrote `$D7FF` within
  its cycle budget, not one that reported a failure code.
